...
export class AddOrganizationComponent implements OnInit {
  ...
  schoolName: string;
  requestType: string;
  headerInfo: String = "Create an Instance for Organization";
  ...
  async ngOnInit() {
    ...
    if (this.requestType != "New") {
      var response = await this.service.GetOrganizationList(queryParameter);
      if (response.statusCode == 0) {
        this._viewModel = response.data;
        console.log(this._viewModel)
        this.AreasByCurrentCitID(response.data.cityID);
        if (this.requestType == 'S') {
          ...
          this.stgId = "";
          this.headerInfo = "Edit an Instance for Organization";
    ...
    var regResponse = await this.service.GetRegOrganization();
    if (regResponse.statusCode == 0)
      this.organizationList = regResponse.data;
    ...
    var organizationType = await this.service.GetOrganizationType();
    if (organizationType.statusCode == 0)
      this.organizationTypeList = organizationType.data;
  }
  async onChangeAreasByID(event) {
    if (this._viewModel.CityID == null) {
      this.areaList = [];
    }
    var response = await this.service.GetAreasByID(event.target.value);
    if (response.statusCode == 0) {
      debugger
  ...
  async AreasByCurrentCitID(id:number) {
    console.log("called by ihsan yes")
    if (this._viewModel.CityID == null) {
      this.areaList = [];
    }
    var response = await this.service.GetAreasByID(id);
    if (response.statusCode == 0) {
      debugger
      this.areaList = response.data;
    }
  ...
  public onChangeObj(id) {
    if (id == 0) { this.organizationFlag = false; this._viewModel.name = this.schoolName; }
    //else if(id == -1){ this.schoolFlag = false; this._viewModel.name = this.schoolName;}
    else if (id) { this.organizationFlag = true; this._viewModel.name = this.organizationList.find(x => x.id == id).name; }
  }
  ...
  async onSubmit() {
    ...
    var formResponse = await this.service.SaveOrganization(this.file, this._viewModel);
    if (formResponse.statusCode == 0)
      this.router.navigate(["/pages/gpn/gpnrequest/approvedorganization"]);
    this.loading = false;
  }
