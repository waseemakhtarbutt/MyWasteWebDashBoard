...
export class AddDonationComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (ID != null) {
      ...
      this._viewModel.title = "Edit Call for Donation";
      this.LoadSubItems(this._viewModel.typeID)
    }
    else {
      this.show = false;
  ...
  async onSubmit() {
    var formResponse = await this.gpnRequestService.SaveDonation(this._viewModel);
    if (formResponse.statusCode == 0)
      this.router.navigate(["/pages/gpn/gpnrequest/donation"]);
