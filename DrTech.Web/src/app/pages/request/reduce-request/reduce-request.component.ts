...
@Component({
  ...
  templateUrl: './reduce-request.component.html',
  styleUrls: ['./reduce-request.component.scss']
})
export class ReduceRequestComponent implements OnInit {
  listViewModel: any[] = [];
  loading = false;
  ...
  async ngOnInit() {
    ...
    var response = await this.requestService.GetAllReduceList();
  ...
  async updateStatus(data: ReduceDTO, ref: NbDialogRef<any>) {
    this.requestService.updateReduceStatusById(data).subscribe(result => {
      this.reloadGrid();
      //this.loadItems();
      ref.close();
    });
  ...
  LoadData() {
    this.requestService.GetReduceList(this.statusId).subscribe(result => {
      if (result.statusCode == 0) {
        result.data.forEach(p => p.pinType = "reduce");
        this.source.load(result.data);
      }
    });
  }
  async onComplaintAction(dialog: TemplateRef<any>, event: any, id: number) {
    var response = await this.requestService.GetReduceById(id);
  ...
  async getSelectedOptionText(event: Event) {
    ...
    if (selectElementText == "Resolved") {
      ...
      // this.resuleModel.greenPoints=10;
    }
    else {
  ...
  public filterChanged(event: Event) {
    ...
    var datalist = this.requestService.GetReduceList(this.statusId).subscribe(result => {
      if (result.statusCode == 0) {
        //this.listViewModel=[];  
        this.skip = 0;
        debugger
