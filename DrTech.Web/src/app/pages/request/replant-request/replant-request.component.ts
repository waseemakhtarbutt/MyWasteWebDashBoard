...
import { ReuseDTO, ReplantDTO } from '../dto';
...
@Component({
  selector: 'ngx-replant-request',
  templateUrl: './replant-request.component.html',
  styleUrls: ['./replant-request.component.scss']
})

export class ReplantRequestComponent implements OnInit {
  listViewModel: any[] = [];
  loading = false;
  ...
  async ngOnInit() {
    ...
    var response = await this.requestService.GetAllReplantList();
  ...
  async updateStatus(data: ReplantDTO, ref: NbDialogRef<any>) {
    this.requestService.updateReplantStatusById(data).subscribe(result => {
      this.reloadGrid();
      //this.loadItems();
      ref.close();
    });
  ...
  async onComplaintAction(dialog: TemplateRef<any>, event: any, id: number) {
    var response = await this.requestService.GetReplantById(id);
