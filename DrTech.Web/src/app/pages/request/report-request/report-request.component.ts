...
export class ReportRequestComponent implements OnInit {
  ...
  async updateStatus(data: ReportDTO, ref: NbDialogRef<any>) {
    this.requestService.updateReportStatusById(data).subscribe(result => {
      this.reloadGrid()
      ref.close();
    });
  }
  ...
  LoadData() {
    this.requestService.GetReportList(this.statusId).subscribe(result => {
      if (result.statusCode == 0) {
        result.data.forEach(p => p.pinType = "report");
        this.source.load(result.data);
      }
    });
  }
  async onComplaintAction(dialog: TemplateRef<any>, event: any, id: number) {
    var response = await this.requestService.GetReportById(id);
  ...
  public filterChanged(event: Event) {
    ...
    debugger
    var datalist = this.requestService.GetReportList(this.statusId).subscribe(result => {
      if (result.statusCode == 0) {
        this.listViewModel = [];
        this.skip = 0;
