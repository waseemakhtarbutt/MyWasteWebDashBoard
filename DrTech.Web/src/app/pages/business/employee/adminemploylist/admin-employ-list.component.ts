...
@Component({
  selector: 'ngx-admin-employ-list',
  templateUrl: './admin-employ-list.component.html',
  styleUrls: ['./admin-employ-list.component.scss'],
  styles: [`
    nb-card {
...
export class AdminEmpListComponent implements OnInit {
  ...
  LoadData() {
    ...
    this.businessService.GetEmployListByRole(false).subscribe(result => {
  ...
  async SuspendEmploy(dialog: NbDialogRef<any>) {
    var response = await this.businessService.SuspendEmploy(this.employId);
