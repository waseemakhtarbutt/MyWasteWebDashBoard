...
@Component({
  ...
  templateUrl: './update-recycle-request-assign.component.html',
  styleUrls: ['../../style/assign-styles.scss'],
})
export class UpdateRecycleRequestAssignComponent implements OnInit {
  mindate = new Date();
  recycleId: string = "";
  ...
  AssignRecycleToDriver() {
    ...
    this.driverService.AssignRecycleToDriver(this._recycleModel).subscribe(result => {
        this.router.navigate(['/pages/request/recycleall']);
    });
  }
