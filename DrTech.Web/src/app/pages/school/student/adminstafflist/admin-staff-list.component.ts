...
export class AdminStaffListComponent implements OnInit {
  ...
  LoadData() {
    ...
    this.schoolService.GetStaffListByRole(false).subscribe(result => {
  ...
  async SuspendStaff(dialog: NbDialogRef<any>) {
    var response = await this.schoolService.SuspendStaff(this.staffId);
