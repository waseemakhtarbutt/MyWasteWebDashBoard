...
export class NbSchoolRegisterComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    const response = await this.userService.GetContactPersonDetail(id);

    if (response.statusCode === 0)
      this.user.contactPerson = response.data;

    const response2 = await this.regService.getSchoolDropdown();
  ...
  async register(){
    ...
    const response = await this.userService.registerSchool(this.user,this.file);
    if (response.data) {
